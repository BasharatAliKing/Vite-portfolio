...
export default function Navbar() {
  ...
  return (
      ...
      <div className={`${Navbar ? "active" : " navbars"} w-full`}>
        <nav className=" text-white z-50 py-1 md:px-10 xl:px-16">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between relative">
            <div className="logo ">
             <Link className="cursor-pointer" to="/home"> <img
                src={logo}
            ...
            <ul
              className={`md:flex items-start bg-[#000] left-0 md:bg-transparent w-full md:items-center absolute md:z-auto z-10   md:w-auto md:static top-14 duration-300 transition-all ease-in py-10 md:py-0 ${
                Open ? "top-[77px] opacity-100" : " top-[-500px]"
              } `}
            >
              {Links.map((link) => (
                <li
